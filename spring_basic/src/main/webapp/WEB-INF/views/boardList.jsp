			...
			<table>
				...
				<c:forEach var="boardDto" items="${list}">
					<tr>
						...
						<td class="title">
							<a href="<c:url value="/board/read${pageHandler.sc.queryString}&bno=${boardDto.bno}"/>">${boardDto.title}</a></td>
				...
				<div class="paging">
					...
					<c:if test="${totalCnt!=null && totalCnt!=0}">
						...
						<c:forEach var="i" begin="${pageHandler.beginPage}" end="${pageHandler.endPage}">
							<a class="page ${i==pageHandler.sc.page? "
								paging-active" : ""}" href="<c:url value="/board/list${pageHandler.sc.getQueryString(i)}"/>">${i}</a>
						</c:forEach>
						<c:if test="${pageHandler.showNext}">
							<a class="page"
								href="<c:url value="/board/list${pageHandler.sc.getQueryString(pageHandler.endPage+1)}"/>">&gt;</a>
						</c:if>
					</c:if>
				</div>
			</div>
